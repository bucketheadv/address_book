...
var AddUser = React.createClass({
  ...
  render: function() {
    ...
    for(var i = 0; i < 3; i++) {
      tagOne.push(
        <TouchableOpacity key={i} onPress={this._select.bind(this, this.state.items[i])}>
  ...
  _addUser: function() {
    ...
    if (!username || !email || !password || !tel) {
      return AlertIOS.alert('提示', '用户名、初始密码、邮箱电话必填，请确认！')
    }
    ...
    var path = Service.v1.regist;
    Util.post(path, {user: obj}, function(data) {
      if (data.status) {
